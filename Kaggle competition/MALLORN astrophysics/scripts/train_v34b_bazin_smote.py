"""
MALLORN v34b: Bazin + SMOTE Oversampling (Techniques #1 + #2)

Combining:
- Technique #1: Bazin parametric features (52 features, 24.8% importance in v34a)
- Technique #2: SMOTE synthetic minority oversampling

SMOTE Strategy:
- Current: 64 TDEs vs 2990 non-TDEs (2.1% positive class)
- Target: Generate 2-3× synthetic TDEs → 128-192 TDE samples
- Apply in feature space (not raw lightcurves)
- Use per-fold to avoid data leakage

Expected gain: +2-5% (PLAsTiCC 2nd place used this)
Target: OOF F1 > 0.70 (from 0.6708 baseline)
"""
...
print("=" * 80, flush=True)
print("MALLORN v34b: Bazin + SMOTE (Techniques #1+2 / 8)", flush=True)
print("=" * 80, flush=True)
...
# SMOTE strategy: oversample to 20% minority class (more balanced)
# From 2.1% → 20% means ~10× increase in TDEs
sampling_strategy = 0.25  # Target 25% TDEs (realistic for small dataset)
print(f"   Target sampling: 25% TDEs (balanced without over-smoothing)", flush=True)
...
submission_path = base_path / 'submissions/submission_v34b_bazin_smote.csv'
...
with open(base_path / 'data/processed/v34b_artifacts.pkl', 'wb') as f:
    pickle.dump(artifacts, f)

print("\n" + "=" * 80, flush=True)
print(f"MALLORN v34b (Bazin + SMOTE) Complete: OOF F1 = {best_f1:.4f}", flush=True)
